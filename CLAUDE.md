...
flowchart LR
    ...
    Traefik -- "/transfers" --> transfer
    Traefik -- "/accounts" --> account
    transfer -- "gRPC ValidateOwnership" --> account
...
- **gRPC (sync)**: transfer → account for ownership validation.
...
| account  | `services/account`  | REST (accounts, lookup) + gRPC server (ValidateOwnership) + Kafka | `account_db` (accounts, ledger_entries, outbox, processed_messages) |
...
Per-service build (from the repo root): `mvn -pl services/<svc> -am clean package` (the `-am` builds the shared libs first). gRPC stubs are generated once in the `fasttrans-grpc-contract` module (from `libs/fasttrans-grpc-contract/src/main/proto/account.proto`) and consumed by transfer + account as a jar dependency — services no longer run protoc themselves.

Per-service test (from inside a service dir): `mvn clean verify` runs unit tests (`*Test`, Surefire — includes ArchUnit `CleanArchitectureTest`), integration tests (`*IT`, Failsafe — Testcontainers, needs Docker), and the JaCoCo ≥90% LINE gate. Use `mvn verify -DskipITs` for a fast unit-only run without Docker.

**CI** (`.github/workflows/ci.yml`, on push/PR to `main`): `backend` job runs `mvn verify` per service (matrix auth/transfer/account); `frontend` job runs `tsc --noEmit` typecheck + `pnpm build`. Testcontainers relies on the Docker preinstalled on `ubuntu-latest`.
