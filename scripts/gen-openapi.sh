...
# gen-openapi.sh
#
# Full pipeline: rebuild auth + transfer + account → wait healthy → fetch specs → merge into docs/openapi.yaml
...
ACCOUNT_TMP="$(mktemp /tmp/fasttrans-account-spec-XXXXXX.json)"

cleanup() {
  rm -f "$AUTH_TMP" "$TRANSFER_TMP" "$ACCOUNT_TMP"
}
trap cleanup EXIT

echo "[1/4] Rebuilding and starting auth + transfer + account services..."
docker compose -f "$PROJECT_ROOT/docker-compose.yml" up -d --build auth transfer account
...
for i in $(seq 1 24); do
  ...
  transfer_up=$(docker compose -f "$PROJECT_ROOT/docker-compose.yml" exec -T transfer \
    wget -qO- http://localhost:8080/actuator/health 2>/dev/null | grep -c '"UP"' || true)
  account_up=$(docker compose -f "$PROJECT_ROOT/docker-compose.yml" exec -T account \
    wget -qO- http://localhost:8080/actuator/health 2>/dev/null | grep -c '"UP"' || true)
  if [ "${auth_up}" -ge 1 ] && [ "${transfer_up}" -ge 1 ] && [ "${account_up}" -ge 1 ]; then
    echo "   All services healthy."
    break
  fi
  if [ "$i" -eq 24 ]; then
    echo "ERROR: Services did not become healthy within 120s." >&2
    docker compose -f "$PROJECT_ROOT/docker-compose.yml" logs --tail=40 auth transfer account >&2
    exit 1
  fi
...
docker compose -f "$PROJECT_ROOT/docker-compose.yml" exec -T account \
  wget -qO- http://localhost:8080/v3/api-docs > "$ACCOUNT_TMP"

echo "   auth spec    : $(wc -c < "$AUTH_TMP") bytes"
echo "   transfer spec: $(wc -c < "$TRANSFER_TMP") bytes"
echo "   account spec : $(wc -c < "$ACCOUNT_TMP") bytes"

echo "[4/4] Merging specs..."
node --experimental-vm-modules "$SCRIPT_DIR/merge-openapi.mjs" "$AUTH_TMP" "$TRANSFER_TMP" "$ACCOUNT_TMP" 2>/dev/null || \
  node "$SCRIPT_DIR/merge-openapi.mjs" "$AUTH_TMP" "$TRANSFER_TMP" "$ACCOUNT_TMP"
